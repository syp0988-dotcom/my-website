...
// 返回主页按钮链接：
// - 开发模式（npm run dev）默认跳回本地主页 http://localhost:8080
// - 构建部署在合并站结构（.../resume3d/site/）时，自动返回站点根目录
// - 如果部署为独立站点，把下面的默认值改成你的主页地址即可
const HOME_URL = (() => {
  if (import.meta.env.DEV) return 'http://localhost:8080/'
  const m = window.location.pathname.match(/(.*)\/resume3d\/site(?:\/.*)?$/)
  return m ? m[1] + '/' : '../../'
})()
...
const COPY = {
  en: {
    title: 'About cyjpj',
    paragraphs: [
      "I'm cyjpj (Sun Yanpei) — an undergraduate at Nanyang Institute of Technology, focused on RAG and AI Agent development. I build production-grade AI applications with LangGraph and full-stack RAG pipelines, and I'm passionate about open source.",
    ],
  },
  zh: {
    title: 'About cyjpj',
    ...
    <>
      ...
      <motion.div className="hero-chrome" style={{ opacity: heroChromeOpacity }} aria-hidden="true">
        ...
        <div className="hero-meta hm-tl">
          <span className="hm-name">cyjpj</span>
      ...
      {/* 返回主页按钮 */}
      <a className="back-home" href={HOME_URL} aria-label="返回主页">
